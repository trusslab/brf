...
func (bpf *Module) GetInsns(name string) ([]byte, error) {
	nameCS := C.CString(name)
	defer C.free(unsafe.Pointer(nameCS))
	//start := (*C.struct_bpf_insn)(C.bpf_function_start(bpf.p, nameCS))
	if bpf.p == nil {
		return nil, fmt.Errorf("Module: no valid program")
	}
	start := (*byte)(C.bpf_function_start(bpf.p, nameCS))
	size := C.int(C.bpf_function_size(bpf.p, nameCS))
	if start == nil {
		return nil, fmt.Errorf("Module: unable to find %s", name)
	}
